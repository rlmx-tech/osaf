...
@pytest.mark.asyncio
async def test_create_incident(client: AsyncClient, verified_user: User):
...
@pytest.mark.asyncio
async def test_create_incident_minimal(client: AsyncClient, admin_user: User):
...
async def test_create_incident_requires_auth(client: AsyncClient):
    """POST /incidents without auth should return 401."""
    response = await client.post("/api/v1/incidents", json=SAMPLE_INCIDENT_PAYLOAD)
    assert response.status_code == 401


@pytest.mark.asyncio
async def test_create_incident_public_user_forbidden(
    client: AsyncClient, test_user: User
):
    """Public users cannot create incidents directly — must use submissions."""
    response = await client.post(
        "/api/v1/incidents",
        json=SAMPLE_INCIDENT_PAYLOAD,
...
@pytest.mark.asyncio
async def test_create_incident_invalid_classification(
...
@pytest.mark.asyncio
async def test_list_incidents_invalid_sort_falls_back(
    client: AsyncClient, sample_incident: Incident
):
    """Invalid sort field should fall back to incident_date without error."""
    response = await client.get("/api/v1/incidents?sort=injected_field; DROP TABLE")
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_update_incident(
    client: AsyncClient, sample_incident: Incident, admin_user: User
):
...
@pytest.mark.asyncio
async def test_update_incident_requires_auth(
    client: AsyncClient, sample_incident: Incident
):
    """PUT /incidents/{id} without auth should return 401."""
    response = await client.put(
        f"/api/v1/incidents/{sample_incident.id}",
...
@pytest.mark.asyncio
async def test_update_incident_not_found(client: AsyncClient, admin_user: User):
    import uuid
...
@pytest.mark.asyncio
async def test_delete_incident_requires_auth(
    client: AsyncClient, sample_incident: Incident
):
    """DELETE /incidents/{id} without auth should return 401."""
    response = await client.delete(f"/api/v1/incidents/{sample_incident.id}")
    assert response.status_code == 401


@pytest.mark.asyncio
async def test_delete_incident_verified_contributor_forbidden(
    client: AsyncClient, sample_incident: Incident, verified_user: User
):
    """Verified contributors can create/update but NOT delete."""
    response = await client.delete(
        f"/api/v1/incidents/{sample_incident.id}",
        headers=auth_header(verified_user),
    )
...
@pytest.mark.asyncio
async def test_delete_incident_not_found(client: AsyncClient, admin_user: User):
    import uuid
